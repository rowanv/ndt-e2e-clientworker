...
class NdtResult(object):
    ...
    '''
    def __init__(self, start_time, end_time, c2s_start_time, s2c_start_time,
        errors, latency=None, upload_speed=None, download_speed=None):
    ...
    def perform_test(self, url, browser, timeout_time=20):
        ...
        try:
            ...
            if e.msg == u'Target URL invalid_url is not well-formed.':
                result_values['errors'].append(TestError(datetime.now(pytz.utc), e.msg))
                self.driver.close()
        ...
        try:
            ...
            element = WebDriverWait(self.driver, timeout=timeout_time).until(
            EC.visibility_of(upload_speed_text))
            ...
            element = WebDriverWait(self.driver, timeout=timeout_time).until(
            EC.visibility_of(download_speed_text))
            ...
            WebDriverWait(self.driver, timeout=timeout_time).until(
                EC.visibility_of(results_text))
            ...
            result_values['errors'].append(TestError(datetime.now(pytz.utc),
